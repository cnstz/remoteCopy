...
string read_all_dir(int fd) {
	int received, n;
	uint32_t rsize;
	if(read(fd, &rsize, sizeof(uint32_t))==-1){
		perror("a");
		exit(1);
	}
...
void* scan_directory(void* newsock) {
	int sock =  (long)newsock;
	cout<<"scan_directory: "<<sock<<endl;
...
bool server_helper::StartThread(int newsock) {
	...
	pthread_t thread_id;
	cout<<"StartThread: "<< newsock<<endl;
	if (pthread_create(&thread_id, NULL, scan_directory,  (void*)((long)newsock)) != 0)
		return false;
	return true;
}
